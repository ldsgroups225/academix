...
describe('isValidJSON', () => {
  ...
  it('should return false for non-string input that cannot be parsed', () => {
    const input = 'invalid json'; //Corrected invalid JSON string
    const result = isValidJSON(input);
    expect(result).toBe(false);
  });

  it('should return true for non-string input that can be parsed', () => {
    const input = 123;
    const result = isValidJSON(String(input)); 
    expect(result).toBe(true);
  });
